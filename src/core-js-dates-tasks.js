...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const REZ = Math.floor((END - START) / (24 * 60 * 60 * 1000));
  return REZ + 1;
}
...
function formatDate(date) {
  ...
  const rez = new Intl.DateTimeFormat('en-US', opt).format(OBJ);
  return rez;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (
    ...
    if (workDays < countWorkDays) {
      rez.push(date.toISOString().split('T')[0].split('-').reverse().join('-'));
      workDays += 1;
